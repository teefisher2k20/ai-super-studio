...
class VideoService:
    ...
    def generate_avatar_video(self, script, avatar_style):
        ...
        return {
            ...
            'status': 'mock_success',
            'note': (
                'Full pipeline requires connecting VoiceService audio -> Wav2Lip'
            )
        }
    ...
    def generate_talking_photo(self, photo, text, audio):
        ...
        try:
            ...
            # If audio file provided, save it. 
            # If text, generate audio first (skip for now)
            if audio:
                ...
                return {
                    'error': (
                        'Text-to-audio for video not yet connected. '
            ...
            if not os.path.exists(checkpoint_path):
                return {
                    ...
                    'error': (
                        'Wav2Lip model weights not found. Please download '
                        'wav2lip.pth to backend/models/wav2lip/checkpoints/'
                    )
                }
