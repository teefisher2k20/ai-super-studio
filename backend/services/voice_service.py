...
class VoiceService:
    ...
    def _generate_sandbox(self, text, voice, model):
        ...
        if model == 'coqui_xtts':
            try:
                ...
                # For implementation, we'll assume a default reference exists.
                # ideally, 'voice' parameter should map to a file path in
                # 'uploads' or 'models/voices'
                ...
                return {
                    'error': (
                        'TTS library not installed. Please run pip install TTS'
                    )
                }
            except Exception as e:
    ...
    def _get_sandbox_voice_path(self, voice_id):
        ...
        default_voice_path = os.path.join(
            default_voice_dir, "default_speaker.wav"
        )

        # If user selected a specific uploaded voice (logic to be added),
        # map it here. For this demo, strictly return default if 'voice_id'
    ...
    def _get_voice_id(self, voice_name):
        ...
        return voice_map.get(voice_name, '21m00Tcm4TlvDq8ikWAM')  # Rachel
